...
KIS_GENERAL_REQUESTS_PER_SECOND=8
KIS_TRADING_REQUEST_INTERVAL_SECONDS=1.2
...
- 일별 성과 저널은 직전 거래일 `15:40 KST` 마감 자산과 당일 `15:40 KST` 마감 자산의 차이를 기준으로 `storage/logs/runtime/daily_performance/YYYY-MM-DD.json`에 자동 생성돼. 실거래는 KIS `주식일별주문체결조회`와 `기간별매매손익현황조회`의 체결가·실현손익·수수료·세금을 원장으로 쓰고, 로컬 주문 이벤트를 주문번호로 병합해 진입계획과 청산 사유를 보존해. 하루의 첫·마지막 사이클이 실패 또는 보류 상태면 그 사이에서 실제 계좌 스냅샷이 있는 첫·마지막 사이클을 사용해. 계좌 스냅샷이 있는 사이클이 하나도 없거나 KIS 조회가 실패하면 가짜 0원 기록을 만들지 않고 해당 날짜 생성을 실패시켜. 실패는 `/api/performance/journal`의 `scheduler`에 남고 스케줄러 스레드는 다음 거래일 실행을 계속 받아. API가 장 마감 뒤 재시작되면 당일 누락분을 한 번 다시 생성해.
...
- `WEALTHPULSE_RESEARCH_LOOP_INTERVAL_SECONDS=60`: 장중 `research-loop`가 성공하거나 일부·전체 종목이 품질 검증에서 탈락한 다음 반복하기까지 대기할 초야. API·인증·할당량·ingest 같은 시스템 실패면 실패 표식을 남기고 `unhealthy` 상태로 대기해.
...
curl -X POST http://127.0.0.1:8001/api/runtime/engine/start \
  -H 'Content-Type: application/json' \
  -d '{"markets":["KOSPI"],"interval_seconds":60,"exit_monitor_interval_seconds":60,"account_sync_interval_seconds":300}'
```
...
런타임 청산은 stop 가격 기반 손절을 먼저 본다. 보유가 `trade_plan.take_profit` 가격 또는 `+12%` 이상에 닿으면 익절한다. 한 번이라도 `+2%` 이상 수익을 본 포지션이 `+0.8%` 이하로 밀리면 수수료·세금·감시 주기 여유를 둔 `본전보호`로 매도한다. 시장가 주문이라 실제 체결 수익을 보장하진 않아. `entry_plan_price`, `stop_loss_price`, `take_profit_price`가 없는 기존 보유 포지션은 자동 손절/익절/본전보호/트레일링익절/기술지표 exit/rotation 교체 매도에서 제외한다. 새 risk plan으로 산 포지션만 `-5%` 이하 `비상손절`을 마지막 안전장치로 쓴다. 추가로 한 번이라도 `+3%` 이상 수익을 본 포지션은 최고 수익률 대비 `3%p` 이상 밀리면 `트레일링익절`로 시장가 매도한다. 이 판단은 기술지표 조회 성공 여부와 분리돼.
신규 매수 후보 탐색과 전략 판단 주기는 `interval_seconds`고 현재 운용값은 `60`초야. 보유 포지션의 가격 기반 손절/익절/본전보호/비상손절/트레일링익절은 `exit_monitor_interval_seconds`로 분리하며 기본 `60`초, 허용 범위 `30~60`초야. 실계좌 전체 잔고 동기화는 `account_sync_interval_seconds`로 따로 관리하며 기본·최솟값은 `300`초야. 60초 전략 사이클 사이에는 마지막 정상 계좌 캐시를 사용하고, 주문이 체결되면 주문 묶음이 끝난 뒤 잔고를 한 번 대조해. 기술지표 보조 exit는 전체 진입 사이클에서만 확인한다. 주문 가능 시간은 KRX 정규장 `09:00~15:30`과 애프터마켓 `15:40~18:00`이야. 프리마켓은 주문하지 않는다. 애프터마켓 주문은 KIS 주문구분 `06` 장후시간외(`15:40~16:00`), `07` 시간외단일가(`16:00~18:00`)로 낸다. 신규 매수는 동적 watchlist active slot만 본다. 리서치 action이 `buy`이거나 품질 좋은 `buy_watch`이고, Layer E가 `review_for_entry`를 내고, 실행 risk plan과 `size_recommendation.quantity > 0`을 통과할 때만 주문 후보가 된다. `hold`는 점수가 높아도 신규 매수로 승격하지 않는다.
...
기본 신규 매수는 하루 1회, 종목별 하루 1회로 제한한다. 저장된 설정이 더 큰 값을 갖고 있어도 런타임에서 하루 1회로 정규화한다. 매도한 종목은 `min_reentry_days=3` 동안 재진입하지 않는다. 이 제한은 신규 매수에만 적용하고 손절·익절 매도는 막지 않는다.
시작 자산은 `LIVE_PERFORMANCE_STARTING_EQUITY_KRW`를 사용한다. 현재 평가금이 시작 자산 대비 `max_total_drawdown_pct=3` 이상 내려가면 `total_drawdown_limit_reached`로 신규 매수를 차단한다. 저장된 설정이 더 큰 값을 갖고 있어도 3%로 정규화하며, 손절·익절 매도는 계속 허용한다.
실계좌 리스크 가드는 KIS `기간별 매매손익`의 당일 확정 실현손익을 사용한다. 이 조회가 실패하거나 날짜가 오늘과 다르거나 필수 손익 필드가 누락되면 `realized_pnl_unavailable`로 모든 신규 매수를 막는다. 확정 손실 합계가 `daily_loss_limit_pct`에 도달하면 `daily_loss_limit_reached`로 차단하며 기본·최댓값은 1%다.
계좌 조회가 실패하거나 총자산이 누락·0·비정상 값이면 엔진 사이클을 즉시 오류 종료해. 단, 장중 전체 사이클의 첫 KIS 잔고조회가 `EGW00215`를 받으면 주문 판단 전에 해당 사이클만 `deferred`로 끝내고 신규 주문을 0건으로 막아. 다음 300초 동기화 시각 전까지는 원장을 다시 조회하지 않고 신규 진입도 계속 차단하며, 캐시와 현재가를 쓰는 가격 기반 자동 청산 감시만 유지해. 3회 연속 동기화 실패면 오래된 계좌로 계속 운용하지 않고 엔진을 오류 정지해. 주문 준비·접수 또는 주문 뒤 잔고 대조에서 발생한 `EGW00215`는 이미 주문이 나갔을 가능성을 배제할 수 없으므로 즉시 오류 정지해. 이런 응답을 1원 계좌나 `100%` 낙폭으로 바꿔서 계속 판단하지 않아. 장중 무주문 전략 사이클은 300초 동기화 시각에만 잔고를 읽고 그 사이에는 마지막 정상 계좌 캐시를 사용해. 모든 운용 시장이 닫혔으면 잔고 원장 호출을 0회로 유지해. 실거래 60초 청산 감시는 마지막 원장 계좌 캐시에 managed 종목별 일반 현재가만 덮어 판단하므로, 청산 신호가 없으면 잔고 원장 호출이 0회야. 청산 신호가 있으면 공식 매도가능수량 조회로 주문 수량을 제한하고, 주문 접수 성공 뒤에만 잔고를 1번 대조해. 시세만 덮은 계좌는 원장 캐시로 저장하지 않아. 실패·보류 알림과 `engine_cycles`에는 직전 성공 ID가 아니라 실제 사이클 ID를 기록해. `/api/runtime/engine/status`, `/api/engine/status`, `/api/engine/summary`는 KIS를 추가 호출하지 않고 저장된 엔진·계좌 상태를 읽어. 보류 중에는 `account_available=true`, `account_fresh=false`, `account_warning`과 `account_sync_deferred=true`를 내리고 홈에는 경고 배너를 띄워. 계좌 신뢰성 오류면 `account_available=false`와 `account_error`도 유지해. 실거래 엔진이 `error`면 원인과 무관하게 홈 최상단에 신규 주문과 자동 청산 감시가 중단됐다는 치명 경보를 띄워.
실거래 주문은 KIS 접수 성공만으로 체결 완료로 보지 않는다. `filled_at`이 없는 실거래 주문은 `submitted` 상태로 남기고, 실제 보유/현금 판단은 이후 계좌 refresh 결과를 기준으로 본다.
교체 매도는 교체 매수 수량이 현재 계좌 기준으로 이미 1주 이상 나올 때만 실행하고, 매도해서 생길 현금을 가정하지 않는다. 매도 전에 일일 buy/sell 제한, 반복 주문 실패 차단, 매수 risk plan, 최종 수량을 전부 사전 검증한다. 하나라도 실패하면 보유 종목을 팔지 않는다. rotation은 하루 1회, 점수 차이 8 이상, 보유 2일 이상일 때만 허용한다. 손절/익절은 이 제한보다 먼저 처리된다.

`paper` 모드는 내부 가상계좌를 쓴다. 가상계좌 상태는 `storage/logs/runtime/accounts/simulated_account_state.json`에 저장돼.

`live` 모드는 KIS를 통해 실계좌 경로를 쓴다. `EXECUTION_MODE=live`를 켜기 전에 `/api/broker/kis/status`, 계좌 상태, 주문 제한을 직접 봐야 해. KOSPI 실계좌 매수는 주문 직전에 KIS 주문가능수량을 조회하고, 시장가 요청이면 현재가 지정가로 바꿔 주문 금액 초과를 줄인다. 매도는 주문 직전에 `매도가능수량조회`(`TTTC8408R`)를 호출해 요청 수량과 현재 주문가능수량 중 작은 값만 한 번 주문해. KIS 호출은 앱키 단위 파일 잠금으로 직렬화하고 `/trading/` 원장 요청은 응답 완료 시각부터 기본 1.2초 간격을 강제해. `EGW00215`는 주식잔고조회 원장 전체의 초당 처리량 초과라 앱 호출 간격만 늘려서는 막을 수 없어. 모든 제한 응답은 자동 재시도하지 않아. `EGW00133`, `EGW00201`과 주문 단계의 `EGW00215`는 즉시 오류 정지하고, 전체 사이클 첫 잔고조회 `EGW00215`만 위의 2회 보류·3회 정지 규칙을 적용해. 요청 감사 기록 자체가 실패해도 브로커 처리 결과가 불명확한 상태로 보고 실거래 엔진을 중단해.
...
- 현금과 포지션 cap
- daily loss limit
- 시작 자산 대비 누적 낙폭 제한(`max_total_drawdown_pct`, 기본·최대 3%)
- KIS 당일 확정 실현손익 가용성(`realized_pnl_unavailable`이면 신규 매수 차단)
