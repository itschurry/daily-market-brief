...
def handle_universe_list(query: dict[str, list[str]]) -> tuple[int, dict]:
    try:
        ...
        if rule_name:
            item = get_universe_snapshot(rule_name, market=market or None, refresh=refresh)
        ...
        if refresh or not rows:
            ...
            for strategy in list_strategies():
                ...
                if key in seen:
                    continue
                seen.add(key)
                refreshed.append(get_universe_snapshot(key[0], market=key[1] or None, refresh=True))
            rows = refreshed
